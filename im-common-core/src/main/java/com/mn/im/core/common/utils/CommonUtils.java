...
//        double dd = Double.parseDouble(d);
     ...
     * @param list
     */
    public static void getEachType(String types, List<String> list){
        if (StringUtils.isNotTrimBlank(types)){
            String[] applyTypeMaps = types.split(",");
            for (String string : applyTypeMaps){
                list.add(string);
            }
        }
    }
    ...
    /**
     * 生成一段24位长度的字符串,最前面用某个字符串拼接
     * @return
     */
    public static String creatRandom24(String string){
        DateFormat bf = new SimpleDateFormat("yyyyMMddHHmmss");//多态
        String nowDateFormat = bf.format(DateUtils.getNowDate());
        String random8 = RandomStringUtils.random(8, "abcdefghijklmnopqrstuvwxyz1234567890");
        return string + nowDateFormat + random8;
    }

    /**
     * 一个页面url对应多个流程节点页面时，区分不同的流程节点，拼接widgetId
     * @param url 访问的url
     * @param str 需要拼接的字符串
     */
    public static String formatWidgetId(String url, String str){
        if(StringUtils.isTrimBlank(url) || StringUtils.isTrimBlank(str)){
            return url;
        }
        StringBuilder result = new StringBuilder(url);
        return result.append("[").append(str).append("]").toString();
    }
}
